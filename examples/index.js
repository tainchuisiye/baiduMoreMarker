// var _getData = require(['getData.js']); var _getData =
// require(['getData.js']); import axios from 'axios'; 创建Map实例

var map = new BMap.Map("allmap");
map.centerAndZoom(new BMap.Point(116.29579, 39.837146), 5);

// _getData.getProviderConter(); 居中放大 启用滚轮放大缩小
map.enableScrollWheelZoom();

var anchScale = {
  anchor: BMAP_ANCHOR_BOTTOM_RIGHT,
  offset: new BMap.Size(55, 30)
};
var anchNavi = {
  anchor: BMAP_ANCHOR_BOTTOM_RIGHT,
  offset: new BMap.Size(0, 27)
};
map.addControl(new BMap.ScaleControl(anchScale));
map.addControl(new BMap.NavigationControl(anchNavi));
...
function MarkerClusterer(map, markers) {
  if (Object.keys(markerClusterer).length === 0) {
    //如果为空，实例化聚合对象
 ...
 *
 */
function _getProvideBoundSimple(map, boundArea) {
  var _bounds = cutBoundsInRange(map.getBounds());
  var _coverProvide = [];

  for (const key in boundArea) {
    if (
      boundArea.hasOwnProperty(key) &&
      Object.keys(boundArea[key]).length === 4
    ) {
      var { minLat, minLng, maxLng, maxLat } = boundArea[key]; //省份边界值
      var { He, Le, Vd, Xd } = _bounds; // 可视区域 lng ：max(HE)/min(Le)  ; lat ：max(Vd)/min(Xd)

      if (minLat < Xd && minLng < Le && maxLat > Vd && maxLng > He) {
        //可视区域所有的点在省份内
        _coverProvide.push(key);
      } else if (
        (minLat < Xd && minLng < Le && maxLat > Xd && maxLng > Le) ||
        (minLat < Xd && minLng < He && maxLat > Xd && maxLng > He) ||
        (minLat < Vd && minLng < Le && maxLat > Vd && maxLng > Le) ||
        (minLat < Vd && minLng < He && maxLat > Vd && maxLng > He)
      ) {
        // 可视区域四个顶点中的某个顶点
        _coverProvide.push(key);
      } else if (minLat > Xd && minLng > Le && maxLat < Vd && maxLng < He) {
        // 省份边界在可视区域内
        _coverProvide.push(key);
      }
    }
  }
  console.log(_coverProvide);
...
function _zoomLevel(map) {
  ...
  if (_level <= 6) {
    ...
    _bounds.forEach(name => {
      if (drawCityed.drawCityName.has(name)) {
        //过滤已经画的
        return;
      } else {
        ...
        _centerPoint[name].pointlist.forEach(({ lng, lat }, index) => {
...
var _getBound = async (name, cb, index) => {
  ...
  await _Boundary.get(name, function(rs) {
    //获取行政区域边界
...
async function getData() {
  await axios.get("./mook/pointDetial.json").then(({ data }) => {
    //按区域划分点
    var cityName = [],
      pointList = [[], []],
      index;
  ...
  await axios.get("./mook/provinceCenter.json").then(({ data }) => {
    // 省份中心点
    cityCenter = data.city;
  });
  await axios.get("./mook/cityCenter.json").then(({ data }) => {
    // 城市中心点
    cityCenter = {
      ...cityCenter,
      ...data
  ...
  axios.get("./mook/provinceBound.json").then(({ data }) => {
    // 省级边界
    providerBound = data;
  });
  axios.get("./mook/cityBound.json").then(({ data }) => {
    // 市级边界
    cityBound = data;
  });
 ...
 */
function getCityCenterAndBoundForbaidu() {
  axios.get("./mook/pointDetial.json").then(async ({ data }) => {
